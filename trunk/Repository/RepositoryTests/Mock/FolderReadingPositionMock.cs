...
namespace RepositoryTests.Mock
{
	...
	public class FolderReadingPositionMock : IFolderReadingPosition, ISerializable
	{
		//private static Version _version = new Version(0, 9, 1, 20110712);
		...
		protected FolderReadingPositionMock(SerializationInfo info, StreamingContext context)
		{
			Version version = (Version)info.GetValue(serializationFieldNameVersion, typeof(Version));
			if (version.CompareTo(PositionVersion) > 0)
			{
				// formatting parameters: component name, expected, then found versions
				throw new SerializationException(string.Format(
					StorageResources.IncompatibleVersion, typeof(FolderReadingPositionMock).Name, PositionVersion, version));
			}
		...
		{
			info.AddValue(serializationFieldNameVersion, PositionVersion, typeof(Version));
		...
		public FolderReadingPositionMock(string folderKey)
		{
		...
		public FolderReadingPositionMock(IRepositoryFolder folder)
		...
		/// </param>
		public FolderReadingPositionMock(IRepositoryFolder folder, DateTime seekTime)
			: this(folder)
		{
			FromSeek(seekTime);
		}
		...
		///		Position to copy
		/// </param>
		public FolderReadingPositionMock(IFolderReadingPosition position)
		{
			FromSeek(position);
		}
		...
		/// </param>
		public void SetEmpty(bfs.Repository.Util.EnumerationDirection readingDirection)
		{
			NumberOfItemsWithTheTimestampRead = 0;
			this.Time = bfs.Repository.Util.TimeComparer.GetComparer(readingDirection).MinValue;
		}
		...
		/// </returns>
		public bool IsFileToBeReadInFull(DateTime firstItemTimestampInFile, bfs.Repository.Util.IDirectedTimeComparison comparison)
		{
